...
pub struct RenderData {
    ...
    pub basis_knot_count: Option<u32>,
    ...
    pub basis_batch_selection_text: String,
    pub basis_batch_edit: BasisEditOverride,
...
impl RenderData {
    pub fn new(max_lod_count: usize) -> Self {
        ...
        Self {
            ...
            basis_batch_selection_text: String::new(),
            basis_batch_edit: BasisEditOverride {
                enabled: true,
    ...
    pub fn toggle_animation_playing(&mut self) -> bool {
        toggle_animation_playing_state(self.has_motion, &mut self.animation_playing)
    }
    ...
    pub fn set_basis_motion(
        ...
        let had_preview = self.basis_bank_preview.is_some();
        self.basis_bank_preview = basis_bank_preview;
        self.basis_knot_count = self
            .basis_bank_preview
            .as_ref()
            .map(|motion| motion.meta.exported_knot_count as u32);
        if !had_preview && self.basis_bank_preview.is_some() {
        ...
        if let Some(motion) = self.basis_bank_preview.as_ref() {
            let basis_count = motion.basis_count;
            ...
            let needs_init = self
                ...
                .map(|edits| {
                    !edits.matches_shape(basis_count, knot_count)
                        || edits.original_knots() != motion.basis_knots.as_slice()
                })
                .unwrap_or(true);
            if needs_init {
                self.basis_knot_edits = Some(BasisKnotEditState::new(
                    motion.basis_knots.clone(),
                    basis_count,
                    knot_count,
                    knot_count,
                ));
        ...
        self.selected_spline_knot =
            clamp_spline_knot(self.selected_spline_knot, self.basis_knot_count);
    }
    pub fn spline_knot_preview_time(&self) -> Option<f32> {
        spline_knot_preview_time(self.selected_spline_knot, self.basis_knot_count, false)
    }
...
fn toggle_animation_playing_state(has_motion: bool, animation_playing: &mut bool) -> bool {
    if has_motion {
...
pub fn advance_animation_phase(
    phase: f32,
    raw_delta_ms: f64,
    animation_speed: f32,
    ...
    let animation_speed = if animation_speed.is_finite() {
        animation_speed.max(0.0)
    } else {
        1.0
    };
    let delta = animation_delta_seconds(raw_delta_ms) * animation_speed / duration_seconds;
    (phase + delta).rem_euclid(1.0)
}
